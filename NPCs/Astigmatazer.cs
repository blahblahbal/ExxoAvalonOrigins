...
namespace ExxoAvalonOrigins.NPCs
{
	public class Astigmatazer : ModNPC
	{
		...
		{
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<Items.SoulofBlight>(), Main.rand.Next(5) + 1, false, 0, false);
                    ...
                    else if (npc.position.Y + npc.height < Main.player[npc.target].position.Y && num496 < 400f)
                    {
                        ...
                        if (npc.ai[3] >= 60f)
                        {
                            ...
                            if (Main.netMode != 1)
                            {
                                ...
                                Projectile.NewProjectile(vector46.X, vector46.Y, num494, num495, 83, num499, 0f, Main.myPlayer, 0f, 0f);
                            }
                        }
                    }
                }
            ...
            else
            {
                ...
                if (npc.ai[1] == 0f)
                {
                    ...
                    if (npc.localAI[1] > 180f && Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
                    {
                        ...
                        Projectile.NewProjectile(vector48.X, vector48.Y, num509, num510, 100, num513, 0f, Main.myPlayer, 0f, 0f);
                        return;
                    }
                    return;
                }
