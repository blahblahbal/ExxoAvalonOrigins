...
{
	public class MechanicalDiggerHead : ModNPC
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Mechanical Digger Head");
        ...
        public override void AI()
        {
            ...
            if (Main.netMode != 1)
            {
                if (npc.ai[0] == 0f)
                {
                    ...
                    for (var num36 = 1; num36 < 20; num36++)
                    {
                        var spawn = 0;
                        if (num36 < 19)
                        {
                            spawn = NPC.NewNPC((int)tmpnpc.position.X + (tmpnpc.width / 2), (int)tmpnpc.position.Y + (tmpnpc.height / 2), ModContent.NPCType<MechanicalDiggerBody>(), tmpnpc.whoAmI);
                        }
                        else if (num36 == 19)
                        {
                            spawn = NPC.NewNPC((int)tmpnpc.position.X + (tmpnpc.width / 2), (int)tmpnpc.position.Y + (tmpnpc.height / 2), 411, tmpnpc.whoAmI);
                        }
                        Main.npc[spawn].realLife = tmpnpc.whoAmI;
                        Main.npc[spawn].ai[2] = tmpnpc.whoAmI;
                        Main.npc[spawn].ai[1] = Previous;
                        Main.npc[Previous].ai[0] = spawn;
                        NetMessage.SendData(23, -1, -1, NetworkText.FromLiteral(""), spawn, 0f, 0f, 0f, 0);
                        Previous = spawn;
                    }
                }
        ...
        {
            return spawnInfo.player.ZoneRockLayerHeight && Main.hardMode && ExxoAvalonOrigins.superHardmode ? 0.083f * ExxoAvalonOriginsGlobalNPC.endoSpawnRate : 0f;
        }
    }
}