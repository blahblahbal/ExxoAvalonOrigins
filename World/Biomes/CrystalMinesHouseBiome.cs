...
namespace ExxoAvalonOrigins.World.Biomes
{
    public class CrystalMinesHouseBiome : MicroBiome
    {
                ...
                if (Main.tile[i, k].active() && Main.tileSolid[(int)Main.tile[i, k].type])
                {
                    ...
                    if (num2 >= 0)
                    {
                        ...
                        while (num3 == 0)
                        {
                            ...
                            if (n2 == 1)
                            {
                                Main.chest[num2].item[2].SetDefaults(ModContent.ItemType<Items.Potions.LuckPotion>(), false);
                                Main.chest[num2].item[2].stack = WorldGen.genRand.Next(2, 4);
                            }
                            if (n2 == 2)
                            {
        ...
        public override bool Place(Point origin, StructureMap structures)
        {
            ...
            list.Add(room);
            if (GenBase._random.NextFloat() > num13 + 0.2f)
            {
                list.Add(rectangle2);
            }
            else
            ...
            {
                WorldUtils.Gen(new Point(item13.X - 10, item13.Y - 10), new Shapes.Rectangle(item13.Width + 20, item13.Height + 20), new Actions.TileScanner(0, TileID.CrystalBlock, (ushort)ModContent.TileType<Tiles.CrystalStone>()).Output(dictionary));
            }
            List<Tuple<BuildData, int>> list6 = new List<Tuple<BuildData, int>>();
            list6.Add(Tuple.Create(BuildData.Crystal, dictionary[(ushort)ModContent.TileType<Tiles.CrystalStone>()] + dictionary[TileID.CrystalBlock]));
                        ...
                        case 1:
                            {
                                ...
                                for (int num9 = -1; num9 < 2; num9++)
                                {
                                    ...
                                    {
                                        GenBase._tiles[num9 + num6, num10 + num8].frameX += 54;
                                    }
                                }
                                break;
                            }
