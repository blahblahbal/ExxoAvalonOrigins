            ...
            for (int num586 = 0; num586 < Main.maxTilesX; num586++)
            {
                ...
                for (int num589 = 0; num589 < Main.maxTilesY; num589++)
                {
                    if (num587 > 0 && !Main.tile[num586, num589].active())
                    {
                        ...
                        Main.tile[num586, num589].type = (ushort)ModContent.TileType<Tiles.Impvines>();
                        num587--;
                    }
                    else
                    {
                    ...
                    if (Main.tile[num586, num589].active() && Main.tile[num586, num589].type == (ushort)ModContent.TileType<Tiles.Impgrass>() && !Main.tile[num586, num589].bottomSlope() && WorldGen.genRand.Next(5) < 3)
                    {
