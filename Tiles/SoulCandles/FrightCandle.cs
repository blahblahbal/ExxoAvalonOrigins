...
namespace ExxoAvalonOrigins.Tiles.SoulCandles
{
    public class FrightCandle : ModTile
    {
        public override void SetDefaults()
        {
            ...
            name.SetDefault("Candle of Fright");
            AddMapEntry(new Color(253, 221, 3), name);
            dustType = ModContent.DustType<Dusts.SoulofFright>();
        }
        public override void KillMultiTile(int i, int j, int frameX, int frameY)
        {
            Item.NewItem(i * 16, j * 16, 16, 16, ModContent.ItemType<Items.Placeable.Light.FrightCandle>());
        }
        public override void MouseOver(int i, int j)
        {
            ...
            player.showItemIcon2 = ModContent.ItemType<Items.Placeable.Light.FrightCandle>();
        }
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            for (int k = 0; k < 7; k++)
            {
                ...
                Main.spriteBatch.Draw(mod.GetTexture("Tiles/SoulCandles/FrightCandle_Flame"), new Vector2(i * 16 - (int)Main.screenPosition.X + offsetX - (width - 16f) / 2f + x, j * 16 - (int)Main.screenPosition.Y + offsetY + y) + zero, new Rectangle(frameX, frameY, width, height), color, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
            }
        }
    }
}
