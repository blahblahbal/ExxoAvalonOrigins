...
namespace ExxoAvalonOrigins.Projectiles.Summon
{
    public class PlatinumDagger : ModProjectile
    {
        ...
        {
            Rectangle dims = ExxoAvalonOrigins.GetDims("Projectiles/Summon/PlatinumDagger");
        ...
        public override void AI()
        {
            Player player = Main.player[projectile.owner];
            ExxoAvalonOriginsModPlayer modPlayer = player.Avalon();
            Main.player[projectile.owner].AddBuff(ModContent.BuffType<Buffs.PlatinumDagger>(), 3600);
            if (projectile.type == ModContent.ProjectileType<PlatinumDagger>())
            {
                if (Main.player[projectile.owner].dead)
                {
                    Main.player[projectile.owner].Avalon().platinumDagger = false;
                }
                if (Main.player[projectile.owner].Avalon().platinumDagger)
                {
            ...
            else
            {
                ...
                if ((projectile.type == ModContent.ProjectileType<PlatinumDagger>()) && projectile.ai[1] == 0f && flag31 && num768 < 500f)
                {
                    projectile.ai[1]++;
