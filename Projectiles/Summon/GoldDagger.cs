...
namespace ExxoAvalonOrigins.Projectiles.Summon
{
    public class GoldDagger : ModProjectile
    {
        ...
        {
            Rectangle dims = ExxoAvalonOrigins.GetDims("Projectiles/Summon/GoldDagger");
        ...
        public override void AI()
        {
            Player player = Main.player[projectile.owner];
            ExxoAvalonOriginsModPlayer modPlayer = player.Avalon();
            Main.player[projectile.owner].AddBuff(ModContent.BuffType<Buffs.GoldDagger>(), 3600);
            if (projectile.type == ModContent.ProjectileType<GoldDagger>())
            {
                if (Main.player[projectile.owner].dead)
                {
                    Main.player[projectile.owner].Avalon().goldDagger = false;
                }
                if (Main.player[projectile.owner].Avalon().goldDagger)
                {
            ...
            else
            {
                ...
                if ((projectile.type == ModContent.ProjectileType<GoldDagger>()) && projectile.ai[1] == 0f && flag31 && num768 < 500f)
                {
                    projectile.ai[1]++;
