...
{
    public class TitaniumDagger : ModProjectile
    {
        ...
        {
            Rectangle dims = ExxoAvalonOrigins.GetDims("Projectiles/Summon/TitaniumDagger");
        ...
        public override void AI()
        {
            Player player = Main.player[projectile.owner];
            ExxoAvalonOriginsModPlayer modPlayer = player.Avalon();
            Main.player[projectile.owner].AddBuff(ModContent.BuffType<Buffs.TitaniumDagger>(), 3600);
            if (projectile.type == ModContent.ProjectileType<TitaniumDagger>())
            {
                if (Main.player[projectile.owner].dead)
                {
                    Main.player[projectile.owner].Avalon().titaniumDagger = false;
                }
                if (Main.player[projectile.owner].Avalon().titaniumDagger)
                {
            ...
            else
            {
                ...
                if ((projectile.type == ModContent.ProjectileType<TitaniumDagger>()) && projectile.ai[1] == 0f && flag31 && num768 < 500f)
                {
                    projectile.ai[1]++;
