    ...
    public class Torch : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            if (projectile.aiStyle == 1)
            {
                ...
                if (!Main.tile[TileX, TileY].active() && (Main.tile[TileX + 1, TileY + 1].active() ||
                    Main.tile[TileX - 1, TileY + 1].active() || Main.tile[TileX + 1, TileY - 1].active() ||
                    Main.tile[TileX - 1, TileY - 1].active()) && !Main.tile[TileX, TileY + 1].active())
                {
                    Item.NewItem((int)projectile.position.X, (int)projectile.position.Y, 16, 16, it);
                    projectile.active = false;
                }
