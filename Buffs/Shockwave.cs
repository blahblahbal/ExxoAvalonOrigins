...
namespace ExxoAvalonOrigins.Buffs
{
    public class Shockwave : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.GetModPlayer<ExxoAvalonOriginsModPlayer>().fallStart_old == -1) player.GetModPlayer<ExxoAvalonOriginsModPlayer>().fallStart_old = player.fallStart;
            int fall_dist = 0;
            if (player.velocity.Y == 0f) // detect landing from a fall
            {
                fall_dist = (int)(((int)(player.position.Y / 16f) - player.GetModPlayer<ExxoAvalonOriginsModPlayer>().fallStart_old) * player.gravDir);
            }
            ...
            if (fall_dist > 3) // just fell
            {
                ...
                { // iterate through NPCs
                    ...
                    { // on screen
                        numOfNPCs++;
                        int multiplier = 2;
                        if (player.GetModPlayer<ExxoAvalonOriginsModPlayer>().earthInsig && Main.hardMode) multiplier = 6;
                        else if (!player.GetModPlayer<ExxoAvalonOriginsModPlayer>().earthInsig && Main.hardMode) multiplier = 4;
                        else if (player.GetModPlayer<ExxoAvalonOriginsModPlayer>().earthInsig && !Main.hardMode) multiplier = 3;
                        else if (!player.GetModPlayer<ExxoAvalonOriginsModPlayer>().earthInsig && !Main.hardMode) multiplier = 2;
                        ...
                        if (player.GetModPlayer<ExxoAvalonOriginsModPlayer>().isOnGround() && numOfNPCs == list.Count - 1) break;
