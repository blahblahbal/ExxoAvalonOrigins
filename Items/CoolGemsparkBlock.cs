...
namespace ExxoAvalonOrigins.Items{	class CoolGemsparkBlock : ModItem	{		public override void SetStaticDefaults()		{			DisplayName.SetDefault("Cool Gemspark Block");		}		public override void SetDefaults()		{			Rectangle dims = ExxoAvalonOrigins.getDims("Items/CoolGemsparkBlock");			item.autoReuse = true;			item.consumable = true;			item.createTile = ModContent.TileType<Tiles.CoolGemsparkBlock>();			item.width = dims.Width;			item.useTurn = true;			item.useTime = 10;			item.useStyle = 1;			item.maxStack = 999;			item.useAnimation = 15;			item.height = dims.Height;		}        public override bool PreDrawInInventory(SpriteBatch spriteBatch, Vector2 position, Rectangle frame, Color drawColor, Color itemColor, Vector2 origin, float scale)
        ...
        {
            Vector2 iPos = item.position - Main.screenPosition;
            spriteBatch.Draw(Main.itemTexture[item.type], item.position - Main.screenPosition, new Rectangle((int)iPos.X, (int)iPos.Y, Main.itemTexture[item.type].Width, Main.itemTexture[item.type].Height), new Color(ExxoAvalonOrigins.gbvR, ExxoAvalonOrigins.gbvG, ExxoAvalonOrigins.gbvB), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
