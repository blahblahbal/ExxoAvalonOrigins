    ...
    public class ExxoAvalonOriginsCollisions
    {
            ...
            Vector2 Velocity, int Width, int Height, bool fireImmune = false)
        {
			...
			for (int i = num; i < num2; i++)
			{
				...
				{
					if (Main.tile[i, j] != null && Main.tile[i, j].slope() == 0 && !Main.tile[i, j].inActive() && Main.tile[i, j].active() && (Main.tile[i, j].type == ModContent.TileType<PoisonSpike>() || Main.tile[i, j].type == ModContent.TileType<Tiles.VenomSpike>() || Main.tile[i, j].type == TileID.Spikes || Main.tile[i, j].type == TileID.WoodenSpikes))
					{
						Vector2 vector2;
